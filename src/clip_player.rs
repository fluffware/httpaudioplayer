...
impl ClipPlayer {
    fn open_audio(&mut self) -> Result<(), pa::error::Error> {
        ...
        let mut settings: pa::OutputStreamSettings<i16> = pa.default_output_stream_settings(
    ...
    pub fn add_clip(&mut self, index: &str, clip: Vec<i16>) {
        self.clips
            .insert(index.to_string(), Arc::new(Mutex::new(clip)));
    }
}
